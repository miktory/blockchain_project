...
const Home: NextPage = () => {
    ...
    const fetchValue = async () => {
        ...
        try {
            if (parametrized)
                result = await selectedContractCommand(inputValue);
            else
                result = await selectedContractCommand();
            ...
            <main className="min-h-screen " style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                <div className="rounded-rectangle-main">
                    <div className="rounded-rectangle" style={{ height: '100%', width: '45%' }}>
                        <div className={`rounded-rectangle ${isOwner ? 'owner' : 'not-owner'}`} style={{ height: '70px', width: '90%' }}>
                            <h1 className="text">{isOwner ? "You are contract owner" : "You are not contract owner"} </h1>
                        </div>
                        <div className={`rounded-rectangle ${isOwner ? 'owner' : 'not-owner'}`} style={{ height: '140px', width: '90%' }}>
                            <h1 className="text">Your address: {connectedAddress} </h1>
                            <h1 className="text">Contract address: {contract.address} </h1>
                        </div>
                        <div className={`rounded-rectangle ${isOwner ? 'owner' : 'not-owner'}`} style={{ height: '100px', width: '90%' }}>
                            <h1>Choose command:</h1>
                            <select className={`rounded-rectangle ${isOwner ? 'owner' : 'not-owner'}`} onChange={handleSelectChange} value={selectedCommand} style={{
                                ...
                                <option value="winningProposal">winningProposal</option> {isOwner && (<option value="endVoting">endVoting</option>)} {isOwner && (<option value="giveRightToVote">giveRightToVote</option>)}
                            </select>
                        </div> {parametrized !== false && selectedCommand !== "" && (<div className={`rounded-rectangle`} style={{
                            display: 'flex', flexDirection: 'row', height: '10%', width: '90%',
                        }}>
                            <label className={'text'}>Parameters</label>
                            <input className={`rounded-rectangle`} style={{
                                backgroundColor: "#000000",
                                height: '35px',
                                width: '200px',
                                flexDirection: 'row',
                        ...
                        </div>)} {selectedCommand !== "" && (<button className="rounded-button" onClick={() => fetchValue()} style={{
                            height: '35px',
                            width: '200px',
                            flexDirection: 'row'
                        }} > Send </button>)}
                    ...
                    <div className="rounded-rectangle" style={{ height: '100%', width: '45%' }}>
                        ...
                        <div className={`rounded-rectangle ${isOwner ? 'owner' : 'not-owner'}`} style={{ height: '80%', width: '90%' }}>
                            <label className="text" style={{ width: '90%', height: '80%' }}>{value}</label>
                        </div>
                    </div>
                </div>
            </main>undefined
